...
function mjc_shortcode_fctn(){
    ...
    // Le joueur a cliqué sur "ENVOYER" 4/4----------------------------------------------------------------------
            ...
            // connection a la BDD
                ...
                // On verifie si l'Email de l'utilisateur a déja été inserer dans la BDD - Empeche les gens de rejouer et donc de tricher
                $query = $wpdb ->get_row("SELECT * FROM $table WHERE user_email = '$email_to_check' ");
                if ( null !== $query ) {
                    echo "<script>alert(\"Votre inscription est invalide. Vous avez déja participé !\")</script>
                    <p id='mjc_heading_3_replay' class='mjc_p'>Vous avez déja joué!</p>
                    <p class='mjc_p'>Inutile de se réinscrire, vous avez déja participé.<br>Un contrôle sera effectué, veuillez lire le règlement.</p>
                    <form class='mjc_text_align_center' method='get'>
                    <input type='submit' name='' value='Revenir au jeu'>";
                    //return true;
                }else{
                // L'email na pas été trouvé on laisse donc les données etre insérées.
                    $datas = array(
                    ...
                    $results = $wpdb->get_results($query);
                        foreach ($results as $result){
                            ...
                            echo "<h2 id='mjc_heading_3_success' class='mjc_p'>Félicitation <b>".$user_firstname."</b> vous êtes inscrit au tirage au sort du concours! </h2>
                            <p class='mjc_p'>Les gagnants seront contactés à l'issue du tirage au sort qui aura lieu le <b>".strftime("%A %d %B %G",strtotime($end_date))."</b>.</p>
                            <form class='mjc_text_align_center' method='get'>
                            <input type='submit' name='' value='Revenir au jeu'>";
                        }
                    //return false;
                ...
                if (!$wpdb){
                    echo "<h2 class='mjc_h2'>Une erreur c'est produite lors de votre inscription, veuillez recommencer./<h2><br><a href='mon_plug_in.php'>Recommencer</a>";
                }
    // Le joueur a cliqué sur "PARTICIPER" mais pas encore envoyer les reponses 3/4----------------------------------------------------------------------
            }else{
            //insertion d'un script pour controler la saisie de date 
                echo "<p id='mjc_heading_2' class='mjc_p'>Veuillez renseigner les informations suivantes:</p>
                <form method='post'>
                    ...
                    <label for='user_name'>Nom* :</label><input type='text' name='user_name' id='user_name' class='mjc_input_front' required/><br>
                    <label for='user_firstname'>Prénom* :</label><input type='text' name='user_firstname' id='user_firstname' class='mjc_input_front' required/><br>
                    <label for='user_email'>Email* :</label><input type='email' name='user_email' id='user_email' class='mjc_input_front' required/><br>
                    <label for='user_birthdate'>Date de naissance* :</label><input type='date' name='user_birthdate' id='user_birthdate' align='center' min='1900-01-01' max='2000-01-01' class='mjc_input_front' required/><br>
                    ...
                    <label for='user_address'>Adresse* :</label><input type='text' name='user_address' id='user_address' class='mjc_input_front' required/><br>
                    <br>
                    <p class='mjc_light'>*Champs obligatoires</p>
                    <div class='mjc_text_align_center'>
                        <input type='submit' name='send_data' value='Envoyer' class='mjc_input_front'/>
                        <p class='mjc_light'>En cliquant sur envoyer vous acceptez et reconnaissez avoir pris connaissance du le règlement.</p>
                    </div>
                </form>
                
                <form class='mjc_text_align_center' method='get'>
                    <input id='mjc_link_button' type='submit' name='' value='Revenir au jeu' class='mjc_input_front'>
                </form>";
            }  
    //  OU le joueur a cliqué sur "VOIR LE REGLEMENT" ----------------------------------------------------------------- 
        ...
        if(isset($_GET['rules'])){
            ...
            echo "
                    <p id='mjc_heading_rules' class='mjc_p'>Règlement</p><br><hr><br>
                    ...
                    personnes s’étant inscrites. Un tirage au sort sera effectué le  <b>".strftime("%A %d %B %G",strtotime($end_date)).".</b> Un seul lot sera attribué par gagnant (même nom, même prénom, même adresse
                    email)</p>
                ...
                <form method='get'>
                <input id='mjc_link_button' type='submit' name='' value='Revenir au jeu'>";
            }
    ...
    // Le joueur n'a pas encore cliqué sur "PARTICIPER" etape 1/3-----------------------------------------------------------------
        ...
        foreach($results as $result){
            ...
            setlocale (LC_TIME, 'fr_FR','fra'); 
            echo "
            <p id='mjc_heading_1' class='mjc_p'>Tentez de gagner <b>".$result->gifts."</b>!</p><br>
                <p class='mjc_p'> Pour jouer, c’est très simple:
                <br>Inscrivez-vous et <b>répondez à notre quiz</b> ! <b><br>".$result->winners_nbr."</b> gagnant(s) sont/seront selectionné(s)<br>
                <p class='mjc_p'>Tirage au sort le <b>".strftime("%A %d %B %G",strtotime($end_date))."</b><br><br>
            ...
            echo " 
                ...
                <form class='mjc_text_align_center' method='get'>
                <input id='mjc_link_button' type='submit' name='rules' value='Voir le réglement'>
                </form>";          
        }
    }
    }
...
function mjc_backoffice_fctn() {
    ...
    if(isset($_POST['game_settings_bdd_data'])){
        echo "
        <div class='mjc_text_align_center'>
            <div id='mjc_backoffice_label'>
                <h1>Mon jeux concours - réglages</h1>
                <h2>Générer ou modifier votre jeux-concours:</h2>
                <p class='mjc_color_white' >Placez votre jeu-concours grace au shortcode : <b>[mon_jeu_concours]</b>.</p>
            </div>
            <p class='mjc_color_red'>Attention votre règlement et les règles du jeux visibles par les participants sont établis dans ces champs :</p>
            <hr>
            <form  method='post'>
                <p>La date de tirage au sort ne peut pas etre anterieur à celle de mise en place et à la date actuelle.</p>
                <label for='start_date'>Date de mise en place : </label><input class='mjc_input' type='date' name='start_date' id='start_date' required/><br>
                <label for='end_date'>Date du tirage au sort : </label><input class='mjc_input' type='date' name='end_date' id='end_date' required/><br>
                <p>Placez un article indefini ou un chiffre avant le gain : exemple une voiture / 1 voiture.</p>
                <label for='gifts'>Lots à gagner :</label><input type='text' name='gifts' id='gifts' class='mjc_input_medium mjc_input' required/><br>
                <p>Definissez ici le nombre de gagnants que l'ordinateur choisira au hasard parmis la liste des participants.</p>
                <label for='winners_nbr'>Nombre de gagnants :</label><input type='number' name='winners_nbr' min='1' id='winners_nbr' class='mjc_input_small mjc_input' required/><br>
                <br>
                <p>Les informations suivantes seront visibles dans le règlement de votre jeu concours:</p>
                <label for='company_name'>Votre nom ou marque: </label><input type='text' name='company_name' id='company_name' class='mjc_input_large mjc_input' required/><br>
                <label for='company_address'>Votre adresse postale </label><input type='text' name='company_address' id='company_address' class='mjc_input_large mjc_input' required/><br>
                <label for='web_site'>L'adresse URL final de votre site :</label><input type='text' name='web_site' id='web_site' class='mjc_input_large mjc_input' required/><br><br>
    ...
    // le client n'a pas appuyé sur "ENVOYER" -> le formlaire s'affiche + les parametres enregistrés
        ...
        echo "
            <div class='mjc_text_align_center'>
                ...
                <hr>
                <p class='mjc_color_red'>Attention : votre règlement et les règles du jeu visibles par les participants sont définis dans ces champs :</p>
                ...
                <form method='post'>
                    ...
                    <label for='gifts'>Lots à gagner :</label><input type='text' name='gifts' id='gifts' class='mjc_input_medium mjc_input' required/><br>
                    <p>Definissez ici le nombre de gagnants que l'ordinateur choisira au hasard parmis la liste des participants.</p>
                    <label for='winners_nbr'>Nombre de gagnants :</label><input type='number' min='1' name='winners_nbr' id='winners_nbr' class='mjc_input_small mjc_input' required/><br>
                    <br>
                    <p>Les informations suivantes seront visibles dans le règlement de votre jeu concours:</p>
                    <label for='company_name'>Votre nom ou marque: </label><input type='text' name='company_name' id='company_name' class='mjc_input_large mjc_input' required/><br>
                    <label for='company_address'>Votre adresse postale </label><input type='text' name='company_address' id='company_address' class='mjc_input_large mjc_input' required/><br>
                    <label for='web_site'>L'adresse URL final de votre site :</label><input type='text' name='web_site' id='web_site' class='mjc_input_large mjc_input' required/><br><br>
                    <input type='submit' name='game_settings_bdd_data' value='Envoyer' class='button button-primary mjc_input' />
                </form><hr> 
            </div>"; 
    ...
    echo "<div class='mjc_text_align_center'>
    ...
    if (isset($results)){
        foreach ($results as $result){
            echo "<p class='mjc_text_align_center'>Date de mise en place prévue: <b>". $result->start_date ."</b><br>Date du tirage au sort prévue: <b>"
    ...
    // Selection au hassar par la fonction RAND(), et nombre de gagnants
        ...
        foreach($results as $result){
                echo "
                    <div id='mjc_winner_background' ><p>Votre grand gagnant est <b>$result->user_firstname
                    $result->user_name</b>(
    ...
    echo "<p class='mjc_color_red'>Attention : cliquez sur <b>Tirage au sort</b> uniquement lorsque vous <b>décidez de cloturer</b> votre concours et une seule une fois!</p>
        <p>
            <form method='post'>
    ...
    $results = $wpdb->get_results($query);
        ...
        echo '<p><table id="contributor" align="center">'; 
        echo "<th>Numéro -</th> <th>Date et heure d'inscription - </th> <th>Civilité - </th> <th>Nom - </th> <th>Prénom - </th> <th>Email - </th> <th>Date d'anniversaire - </th> <th>Adresse</th>"; //table headers
        foreach($results as $result){
            echo'<tr>';
    ...
    // <!- ************************* copyright ***************************************
    // ************** Plugin: Mon jeu concours par Frederic Castel *************
    // ******************************************************************************** -->
